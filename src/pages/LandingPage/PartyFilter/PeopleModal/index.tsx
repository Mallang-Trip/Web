...
function PeopleModal({ showModal, setShowModal }: Props) {
  const dispatch = useDispatch();
  const modalRef = useRef<HTMLDivElement | null>(null);
  const num = useSelector((state: RootState) => state.partyFilter.num);
  const [modalNum, setModalNum] = useState(1);

  const setIncrease = useCallback(() => setModalNum(modalNum + 1), [modalNum]);
  const setDecrease = useCallback(() => setModalNum(modalNum - 1), [modalNum]);
  ...
  return createPortal(
    ...
    >
      <div
        className={clsx(
          "mx-auto mt-auto shadow w-full rounded-xl duration-500",
...
export default memo(PeopleModal);
