...
interface Props extends MallangTalkInfo, ReceiptInfo {
  startDate: string;
  capacity: number;
  headcount: number;
  setMallangTalkInfo: Dispatch<SetStateAction<MallangTalkInfo>>;
  setShowMallangTalkModal: Dispatch<SetStateAction<boolean>>;
  ...
  return (
    <div className="flex flex-col px-5 py-6 font-medium border border-gray-300 rounded-3xl mb-4">
      ...
      <div className="flex justify-between w-full">
        <div className="flex flex-col text-sm text-gray500 w-24">
          드라이버
          <div
            className={clsx("mt-1.5", {
              "text-primary":
                driverPenaltyAmount &&
                driverPenaltyStatus === "PENALTY_PAYMENT_COMPLETE",
              "text-[#FF0000]":
                driverPenaltyAmount &&
                driverPenaltyStatus !== "PENALTY_PAYMENT_COMPLETE",
              "text-gray700": !driverPenaltyAmount,
            })}
          >
            {driverName} 드라이버
            {driverPenaltyAmount && (
        ...
        <div className="flex flex-col text-sm text-gray500 w-72">
          여행자 정보
